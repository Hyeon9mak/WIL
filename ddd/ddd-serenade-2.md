...
> --------------------
> 여기 부분 강의영상 참고해서 수정할 것  
Mockito를 사용하면, 스터빙 대상이 되는 객체의 구현방식이 바뀌면 테스트 코드의 given 또한 모두 바뀌어야 한다.
SpringBootTest를 사용하면 ~~ 한다는 단점이 있다.
> --------------------
...
`BOUNDED CONTEXT`를 `MSA`로 바꿔서 읽을 수도 있다.
...
> 제이슨이 공유해주시는 추가 강의링크 확인해서 내용 보충
