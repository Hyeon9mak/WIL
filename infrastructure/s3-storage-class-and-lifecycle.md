...
#### Parquet 은 다량의 row 를 한꺼번에 압축 할 때 효율적이다.
Parquet **열 지향(Columnar) 저장 방식**을 사용하기 때문. 단일 Row를 압축하여 업로드할 경우, 실제 데이터보다 컬럼 스키마와 통계 정보 같은 **메타데이터가 차지하는 비중이 비정상적으로 커지는 '배보다 배꼽이 더 큰' 상황**이 발생한다. 반면 10만 건 단위로 데이터를 묶으면 동일 컬럼 내의 중복된 값들을 사전 인코딩(Dictionary Encoding)이나 델타 인코딩 등으로 압축하여 용량을 극적으로 줄일 수 있다. 

작은 파일이 무수히 많아지면 S3 저장 비용보다 **PutObject 요청 비용이 더 커질 수 있으며**, 이후 Athena나 Spark로 데이터를 조회할 때도 수많은 파일을 여닫는 오버헤드로 인해 쿼리 성능이 심각하게 저하된다. 따라서 시스템의 메모리(OOM)가 허용하는 한도 내에서 적절한 Chunk 단위로 데이터를 그룹화하여 하나의 파일로 만드는 과정이 반드시 필요하다.
