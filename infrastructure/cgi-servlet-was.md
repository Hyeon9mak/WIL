...
## DD(Deployment Descriptor, XML)을 통한 선언적 관리
를! 통해서 컨테이너 및 서블릿을 관리한다고 책에서 이야기하지만, 현재는 디스패쳐 서블릿을 사용하기 때문에 
DD보다 디스패쳐 서블릿 관련 자바 클래스로 관리가 쉬워졌다 - 코기 테코톡 다시 시청하면 좋을듯.
...
결합이 되지 않거나 디자인 팀과 개발 팀의 성과물을 연결하는 과정에서 불협화음이 일어나는 문제가 잦았다. 

그래서 재사용이라는 발상을 떠올려서, 재사용할 수 있는 부분을 늘려서 애플리케이션 개발을 용이하게 하는 프레임워크가 등장했다. 톰캣.

그렇다면 톰캣과 같은 서블릿 컨테이너가 꼭 필요한가? 재사용성을 조금 포기한다면 서블릿 컨테이너 없이도 서블릿만 이용해서 동적인 HTML 전달이 가능한 것 아닌가? 
라는 의문을 가질 수 있다. 우선 서블릿 컨테이너가 주는 혜택을 알아보자.

### 통신 지원
컨테이너는 서블릿과 웹 서버가 서로 통신할 수 있는 API를 제공해준다. 
즉, 개발자가 직접 ServerSocket을 만들고, 특정 PORT를 리스닝하고, 연결 요청이 들어오면 스트림을 생성하는 등 
복잡한 과정을 거칠 필요 없이 컨테이너가 제공하는 API를 이용하면 끝난다.
컨테이너 덕분에 개발자가 웹 서버와 서블릿 간의 통신을 알 필요가 없게 된다.

### 라이프사이클 관리
컨테이너는 서블릿의 라이프사이클을 관리한다. 
서블릿 클래스를 로딩하여 인스턴스화 하고, 초기화하고, 요청에 따른 적절한 서블릿 메서드를 호출하는 작업까지 모두 
컨테이너가 자동으로 수행해준다. 서블릿이 생명을 다한 순간에는 적절하게 GC를 호출하기 까지 한다.

### 멀티스레딩 지원
컨테이너는 요청마다 새로운 자바 스레드를 하나 만든다. 그 스레드로 적절한 HTTP 서비스 메서드를 실행하면 끝난다.
스레드 안전성에 대해서도 개발자가 신경쓸 필요가 없다.

### 선언적 보안 관리
컨테이너를 사용하면 XML 파일에다가 보안 내용을 등록해두기만 하면 된다. 
보안에 대해 수정할 일이 생기더라도, 자바 소스 코드를 수정하고 재컴파일 할 필요 없이 
XML 파일만 수정하면 된다.

### JSP 지원
JSP를 사용해서 동적 HTML 파일을 지원한다고 했다. JSP 파일을 누가 실제 자바 코드로 변환할까?
바로 컨테이너다.
...
> 그러나 정확히 이야기하면 톰캣은 온전히 애플리케이션 서버라고 부를 수 없다. 애플리케이션 서버로 불리기 위해선 EJB(Enterprise Java Bean)가 
필요한데, 톰캣은 EJB 기술이 적용되어 있지 않고 서블릿과 JSP를 위한 런타임 환경만 제공하기 때문이다. EJB를 대신하게 된 것이 바로 Spring이다!

> 그렇다면 서블릿 컨테이너 기능과 EJB 기능을 제외한 나머지, 웹 애플리케이션으로 불릴 수 있는 기능들은 누가 처리하고 있는가?
> Spring 에서 Spring boot 로 넘어오면서, Spring boot가 나머지 JavaEE 스펙들을 모두 충족해주고 있다.

> 이 부분에 대해서는 논란이 있다. 이에 대해 정리한 [톰캣은 WAS일까?](is-tomcat-was.md)를 참고하자.
